...
public:
	...
	//ofTrueTypeFont  mtreeFont;

	//offscreen draw batch
	...
	Tree() :
		...
		//mtreeFont.load("AmsiPro-Ultra.ttf", 38, true, true);
	}
	...
	void setup(std::vector<Particle>& all) {
		int n = all.size();
		if(n > 0) {
			// find boundaries
			minX = all[0].x;
			...
			float halfSide = glm::max(diffX, diffY) / 2;
			minX = midX - halfSide;
			maxX = midX + halfSide;
			minY = midY - halfSide;
			maxY = midY + halfSide;
	...
	std::vector<Particle*> getNeighbors(float targetX, float targetY, float radius) {
		std::vector<Particle*> intersection, neighbors;
	...
	void getIntersection(std::vector<Particle*>& intersection, float targetX, float targetY, float radius) {
